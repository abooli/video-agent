...
# 2. WhisperX 本地转录（无需上传，无需 API Key）
#    language 参数: "en" 英语 | "zh" 中文 | "auto" 自动检测（默认）
SKILL_DIR="~/.claude/skills/videocut/剪口播"
DICT_FILE="~/.claude/skills/videocut/字幕/词典.txt"
python "$SKILL_DIR/scripts/whisperx_transcribe.py" audio.mp3 auto "$DICT_FILE"
...
```bash
node "$SKILL_DIR/scripts/generate_subtitles.js" whisperx_result.json
...
WhisperX 无需 API Key，完全本地运行。首次使用会自动下载 large-v2 模型（约 3GB）。

语言参数：
- `auto` — 自动检测（默认，适合不确定语言时）
- `en` — 英语（更快，跳过语言检测）
- `zh` — 中文
